...
Rozdział~\ref{r:definicje} objaśnia definicje użyte w pracy licencjackiej.
Rozdział~\ref{r:konkurencja} stanowi przegląd oraz analizę konkurencyjnych rozwiązań. W rozdziale~\ref{r:arch} przedstawiono całościową architekturę systemu oraz wywzwania i problemy nopotkane w trakcie implementacji funkcjonalności. Infrastruktura serwera została szczegółowa opisana w rozdziale~\ref{r:infrastruktura}. Rozdział~\ref{r:usecase} szczegółowo opisuje najważniejsze przypadki użycia aplikacji.  Rozdział~\ref{r:zarzadzanieProjektem} poświęcony jest metodyce tworzenia projektu oraz podziale pracy w zespole. Końcowe rozdziały zawierają: spis zawartości płyty dołączonej do pracy oraz krótkie podsumowanie projektu.
